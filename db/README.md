...
| Queries / models | GORM |
| Apply in CI / Neon | `go run ./cmd/migrate up` with **direct** Neon URL (`DATABASE_URL_MIGRATE`) |
| Apply locally | `make migrate` |
| Cloud Run startup | **does not** migrate (avoids multi-instance races) |

Goose uses Postgres session advisory locks. Neon **pooler** URLs often break that — use the direct host for migrate, pooled URL only for the app (`DATABASE_URL` in Secret Manager).
